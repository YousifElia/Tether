...
const FileBrowser = (() => {
  ...
  async function remove(name) {
    ...
    try {
      ...
      if (res.ok) { setStatus(`Deleted ${name}.`, 'ok'); await refresh(); }
      else if (res.status === 404) { setStatus('File no longer exists.', 'error'); await refresh(); }
  ...
  function wire() {
    ...
    els.input.addEventListener('change', () => {
      if (els.input.files.length) { upload(els.input.files); els.input.value = ''; }
    });
    // Drag-and-drop (desktop). The label still opens the picker on tap (mobile).
    ['dragenter', 'dragover'].forEach((ev) =>
      els.drop.addEventListener(ev, (e) => { e.preventDefault(); if (!ro) els.drop.classList.add('dragover'); })
    );
    ['dragleave', 'dragend', 'drop'].forEach((ev) =>
      els.drop.addEventListener(ev, () => els.drop.classList.remove('dragover'))
    );
    els.drop.addEventListener('drop', (e) => {
      e.preventDefault();
      if (!ro && e.dataTransfer && e.dataTransfer.files.length) upload(e.dataTransfer.files);
    });
  }

  function setReadOnly(value) {
    ro = value;
    els.upload.hidden = ro;          // only owners see the upload zone
    if (enabled && !els.panel.hidden) render(); // refresh delete buttons
  }

  return { init: probeAndMaybeEnable, setReadOnly };
})();
